...
import CaptureSound from '/capture-sound.wav'
...
export const ChessBoard = memo(
  ({
    ...
    console.log("checking all the values >>> ", "gameOver= ", gameOver, "isFlipped=", isFlipped, )
    return (
      <>
        {gameOver && <Confetti />}
              ...
              return (
                <div key={i} className="flex relative">
                  ...
                  {(isFlipped ? row.slice().reverse() : row).map((square, j) => {
                   ...
                   // Add this line inside the inner map loop
                   console.log(`Square: ${String.fromCharCode(97 + j) + i}, i: ${i}, j: ${j},isHighlightedSquare: ${isHighlightedSquare}, isMainBoxColor: ${isMainBoxColor}, isHighlighetSquare: ${isRightClickedSquare}, isKingInCheckSquare:${isKingInCheckSquare}`);
                    return (
                      <div
                        onClick={() => {
                          if (!started) {
                        ...
                        key={j}
                        className={`${isRightClickedSquare ? (isMainBoxColor ? 'bg-[#CF664E]' : 'bg-[#E87764]') : isKingInCheckSquare ? 'bg-[#FF6347]' : isHighlightedSquare ? `${isMainBoxColor ? 'bg-[#BBCB45]' : 'bg-[#F4F687]'}` : isMainBoxColor ? 'bg-navy' : 'bg-white'} ${''}`}
                        // className={isMainBoxColor ? 'bg-navy' : 'bg-white'}
                        onContextMenu={(e) => {
                          e.preventDefault();
                        }}
                        onMouseDown={(e) => {
